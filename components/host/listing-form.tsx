...
const STEPS = ["category", "location", "details", "amenitiesStep", "images", "pricing"] as const;

const STEP_FIELDS: Record<(typeof STEPS)[number], string[]> = {
  category: ["category"],
  location: ["city", "province", "address"],
  details: ["title", "description", "guestCount", "bedroomCount", "bedCount", "bathroomCount"],
...
export function ListingForm({ listingId, defaultValues }: ListingFormProps) {
  ...
  async function nextStep() {
    ...
    if (step === "images") {
      // Materialize gallery urls into the schema fields.
    ...
    <form onSubmit={onSubmit} className="mx-auto max-w-3xl space-y-8">
          ...
          <li key={s}>
            <button
              ...
              className={cn(
                ...
                i === stepIndex && "bg-foreground text-background font-semibold",
            ...
            {CATEGORIES.map(({ value, icon: Icon }) => (
              <button
                ...
                className={cn(
                  ...
                  form.watch("category") === value && "border-foreground bg-accent font-semibold",
                )}
              >
            ...
            {counterFields.map(({ name, min, max }) => (
              <div key={name} className="grid gap-1.5">
                ...
                <Input
                  ...
                  min={min}
                  max={max}
                  {...form.register(name, { setValueAs: (v) => (v === "" ? undefined : Number(v)) })}
                  dir="ltr"
                ...
                <button
                  ...
                  onClick={() => {
                    const nextValue = checked
                      ...
                      : [...current, key];
                    form.setValue("amenities", nextValue as never, { shouldValidate: false });
                  }}
                  className={cn(
        ...
        <section className="max-w-sm space-y-2">
          ...
          {form.watch("pricePerNight") != null && (
            <p className="text-sm text-muted-foreground" dir="auto">
              ≈{" "}
              {new Intl.NumberFormat(locale === "fa" ? "fa-IR" : locale === "ar" ? "ar" : "en-US").format(
                Number(form.watch("pricePerNight")),
              )}
            </p>
          )}
        </section>
      ...
      <div className="flex items-center justify-between gap-3 border-t pt-6">
        ...
        {stepIndex < STEPS.length - 1 ? (
          <Button type="button" onClick={() => void nextStep()} disabled={pending}>
            {tc("next")}
          </Button>
        ) : (
          <Button
